...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $id = $conn->real_escape_string($id); // Escape the ID

    // Fetch curriculum details
    $qry = $conn->query("SELECT * FROM curriculum_list WHERE `status` = 1 AND id = '{$id}'");
    if ($qry->num_rows > 0) {
        $curriculum = $qry->fetch_assoc();
    } else {
    ...
    <div class="col-12">
        <div class="card card-outline card-primary shadow rounded-0" style="border-color: #800000;">
            <div class="card-body rounded-0">
                <h2>Program Studies of <?= htmlspecialchars($curriculum['name'] ?? "") ?></h2>
                <p><small><?= htmlspecialchars($curriculum['description'] ?? "") ?></small></p>
                <hr class="bg-navy">
                ...
                <div class="filter-container mb-3">
                    ...
                    <select id="filter-letter" class="form-control mt-2">
                        ...
                        <?php foreach (range('A', 'Z') as $letter): ?>
                            <option value="<?= htmlspecialchars($letter) ?>" <?= $letter_filter === $letter ? 'selected' : '' ?>>
                ...
                // Fetch students data
                $students = $conn->query("SELECT * FROM student_list WHERE id IN (SELECT student_id FROM archive_list WHERE `status` = 1 AND curriculum_id = '{$id}')");
                ...
                $count_all_query = $conn->query("SELECT COUNT(*) FROM archive_list WHERE `status` = 1 AND curriculum_id = '{$id}' {$year_filter_sql} {$letter_filter_sql}");
                $count_all = $count_all_query->fetch_row()[0];
                $pages = ceil($count_all / $limit);

                // Fetch archives with filters and pagination, sorted by title
                $archives_query = $conn->query("SELECT * FROM archive_list WHERE `status` = 1 AND curriculum_id = '{$id}' {$year_filter_sql} {$letter_filter_sql} ORDER BY title ASC LIMIT {$limit} OFFSET {$offset}"); ?>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <ul class="pagination pagination-sm m-0 float-right">
                                <li class="page-item"><a class="page-link" href="./?page=projects_per_curriculum&id=<?= $id ?>&p=<?= max($page - 1, 1) ?>" <?= $page == 1 ? 'disabled' : '' ?>>«</a></li>
                                <?php for ($i = 1; $i <= $pages; $i++): ?>
                                    <li class="page-item"><a class="page-link <?= $page == $i ? 'active' : '' ?>" href="./?page=projects_per_curriculum&id=<?= $id ?>&p=<?= $i ?>"><?= $i ?></a></li>
                                <?php endfor; ?>
                                <li class="page-item"><a class="page-link" href="./?page=projects_per_curriculum&id=<?= $id ?>&p=<?= min($page + 1, $pages) ?>" <?= $page == $pages || $pages <= 1 ? 'disabled' : '' ?>>»</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
